...
contract LinearVRGDALibWrapper {
    ...
    function getPerTimeUnit(uint256 _count, uint256 _durationSeconds) external pure returns (SD59x18) {
        SD59x18 result = LinearVRGDALib.getPerTimeUnit(_count, _durationSeconds);
        return result;
    }

    function getVRGDAPrice(uint256 _targetPrice, uint256 _timeSinceStart, uint256 _sold, SD59x18 _perTimeUnit, SD59x18 _decayConstant) external pure returns (uint256) {
